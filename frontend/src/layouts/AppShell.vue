  ...
  <div class="min-h-screen bg-slate-50 font-sans flex text-slate-900">
    ...
    <aside class="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 sticky top-0 h-screen">
      ...
      <nav class="flex-1 px-4 space-y-1">
        <button v-for="item in navItems" :key="item.id"
          ...
          class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500"
          :class="activeView === item.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'"
        >
          <AppIcon :name="item.icon" class="w-5 h-5" aria-hidden="true" />
          {{ item.label }}
        </button>
      </nav>

      <div class="p-4 border-t border-slate-100">
        <button @click="$emit('logout')" class="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-red-50 transition-colors group focus:outline-none focus:ring-2 focus:ring-red-200" aria-label="Cerrar sessió">
          ...
          <AppIcon name="logout" class="w-4 h-4 text-slate-500 group-hover:text-red-500 transition-colors" aria-hidden="true" />
        </button>
      </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col min-w-0 pb-20 md:pb-0">
      <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-30">
        ...
        <div class="flex items-center gap-4">
          <button class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-500 hover:text-indigo-600 transition-colors" aria-label="Notificacions">
            <AppIcon name="bell" class="w-5 h-5" aria-hidden="true" />
          </button>
        </div>
      </header>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-slate-200 h-16 flex items-center justify-around px-2 z-40">
      <button v-for="item in navItems" :key="item.id" @click="changeView(item.id)"
        ...
        :class="activeView === item.id ? 'text-indigo-600' : 'text-slate-500'"
      >
