...
describe('NotificacionsService', () => {
    ...
    mockConfigService = {
      get: jest.fn((key: string) => {
        const config: Record<string, string> = {
          VAPID_PUBLIC_KEY: 'BAYpv9NMlZYady-ke2W-h4Zlcf0sn0hCVPWotImSKHnmo8BWPzCJJbI3ZUtV2-Sx3nInjqfzYAb-BzYw2gFuN7U',
  ...
  describe('subscriure', () => {
    it('should create a new subscription when it does not exist', async () => {
      ...
      const mockSubscription = { endpoint: 'test-endpoint', keys: { p256dh: 'test', auth: 'test' } };
      ...
      const result = await service.subscriure(mockUser, mockSubscription, userAgent);
    ...
    it('should return existing subscription message when subscription exists', async () => {
      ...
      const result = await service.subscriure(mockUser, mockSubscription, 'Mozilla');
  ...
  describe('getVapidPublicKey', () => {
    it('should return the VAPID public key', () => {
      ...
      expect(publicKey).toBe('BAYpv9NMlZYady-ke2W-h4Zlcf0sn0hCVPWotImSKHnmo8BWPzCJJbI3ZUtV2-Sx3nInjqfzYAb-BzYw2gFuN7U');
    });
  ...
  describe('enviarNotificacioFamilia', () => {
    ...
    it('should send notifications to tutors with push subscriptions', async () => {
      ...
      mockRepository.find.mockResolvedValue([
        { id: 1, usuariId: 2, tokenSubscripcio: JSON.stringify(mockSubscription) },
      ]);
