...
export class NotificacionsController {
    ...
    @Post('subscriure')
    async subscriure(@Request() req: any, @Body() subscription: any, @Headers('user-agent') userAgent: string) {
        // req.user ve del JwtToken
        return this.notificacionsService.subscriure(req.user, subscription, userAgent);
    }
}
