...
import { DadesFamiliars, Familiar } from '../users/interfaces/dades-usuari.interface'; // Ensure interface is imported
...
export class SeedService {
    ...
    async executarSeed() {
        try {
            ...
            const assignacions = await this.crearAssignacions(primerGrup, professors, assignatures);
            ...
            if (dadesFamiliaresArray && Array.isArray(dadesFamiliaresArray)) {
                for (const fam of dadesFamiliaresArray) {
                    ...
                    if (!famEmail || !isValidEmail(famEmail)) {
                        ...
                        const cleanName = fam.nom_complet.toLowerCase().replace(/\s+/g, '.');
                        famEmail = `${cleanName}@familia.adsum.cat`;
                    }

                    // Check if parent user already exists
                    let tutor = await this.usuariRepo.findOne({ where: { email: famEmail } });
    ...
    private async crearAssignacions(grup: Grup, professors: Usuari[], assignatures: Assignatura[]) {
